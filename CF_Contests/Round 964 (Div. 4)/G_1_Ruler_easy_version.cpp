...
void solve(){
    // since it is an interactive problem, we need to flush, hence use endl instead of '\n'

    // As we are given that max query we can ask is 10, therefore we apply binary search on the range of x
    int l=2, r=1e3;
    while(l<r){
        int mid= (l+r)/2;
        cout<<"? 1 "<<" "<<mid<<endl;
