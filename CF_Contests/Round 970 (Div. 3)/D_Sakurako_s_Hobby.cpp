...
void solve(){
    ...
    fore(i,0,n)cin>>v[i], v[i]--;       // taking it as zero based index
    string s; cin>>s;

    vector<int>dp(n,-1);        // stores the cnt of Black balls reached

    for(int i=0; i<n; i++){
        if(dp[i]!=-1)continue;  // already calculated

        int cnt=0;
        int j=i;

        if(s[j]=='0')cnt++;         // current element is itself Black
        j=v[j];
