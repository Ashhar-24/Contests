...
void solve(){
    int n,k; cin>>n>>k;
    (((n*(n+1)/2 )- ((n-k)*(n-k+1)/2))%2==0)? yes: no;

    /*
         As i^i will have same parity as i. So in order to know the parity of sum of i^i till, it is equivalent to finding
         sum of i's. And as k will tell us how much towards left should we move from 'n'.
